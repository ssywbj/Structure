package com.structure.wallpaper.basic.utils;

public class Constants {
    public static final String ACTION_UPDATE_WATCH_FACE = "action_update_watch_face";
    public static final String KEY_DATA_COMPONENT_NAME = "key_data_component_name";
    public static final String KEY_ICON_FACE = "key_icon_face";
    public static final String KEY_SELECTED_COMPONENT = "key_selected_component";
    public static final String REGEX_SPILT = "#";

    public static final String ACTION_UPDATE_FACE_WIDGET = "action_update_face_widget";
    public static final String KEY_FACE_WIDGET_PKG = "key_face_widget_pkg";
    public static final String KEY_FACE_WIDGET_CLZ = "key_face_widget_clz";
}
