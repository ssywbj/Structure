...
public class Parameterized3Test {
    ...
    private final int expected; //预期值
    ...
    public static Iterable<Integer[]> parameters() {
        ...
        return Arrays.asList(new Integer[][]{{7, 5, 11}, {7, 4, 11}, {7, 3, 10}, {7, 2, 12}}); //一维数组里的值表示传入的实参，二维数组里的值表示传入的实参有多少组。
    }

    public Parameterized3Test(int one, int two, int expected) {
        this.one = one;
        this.two = two;
    ...
    public void testAdd() {
        ...
        Assert.assertEquals(this.expected, add(one, two));
    }
